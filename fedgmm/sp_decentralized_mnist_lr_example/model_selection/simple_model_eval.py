...
class SGDSimpleModelEval(StandardSimpleModelEval):
    ...
    def do_training_update(self, f, f_optimizer, x_train, y_train):
        ...
        for _ in range(num_batch):
            ...
            # loss.backward()
            # f_optimizer.step()
            predictions = f(x_batch).clone()  # Clone the result to avoid modifying the model's output
            loss = ((predictions - y_batch) ** 2).mean()  # Compute the loss
    
    # Backpropagation
            loss_clone = loss.clone()  # Clone the loss to avoid in-place modification of gradients
            loss_clone.backward()  # Backpropagation step (calculates gradients)

    # Optimizer step (this is inherently in-place, but unavoidable for parameter updates)
            f_optimizer.step()
