...
class LeakySoftmaxCNN(nn.Module):
    ...
    # Calculating weights and probabilities
        leaky_class_weights = F.leaky_relu(self.linear_2(data))
        class_probs = F.softmax(leaky_class_weights, dim=1)  # This might need correction
        total_weight = leaky_class_weights.sum(1).view(-1, 1)
        data = leaky_class_weights * 0.01 + total_weight * class_probs * 0.99
    ...
    # Conditional batch normalization
        if self.training:
         if data.size(0) > 1:
            ...
            # Apply batch normalization without updating running stats
            data = F.batch_norm(
                data, running_mean=None, running_var=None,
                weight=self.bn.weight, bias=self.bn.bias, training=True)
        else:
        # Use stored running stats for evaluation
          data = self.bn(data)
        return self.linear_3(data)
...
class DefaultCNN(LeakySoftmaxCNN):
    ...
    def forward(self, data):
        # data = data.view(data.shape[0], 3, 32, 32)
        data = data.view(data.shape[0],1,28,28)
        # data = data.view(data.shape[0])
